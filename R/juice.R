...
# 2. 각 번호당 뽑혔던 빈도를 가중치로 하여 가장 뽑힐 확률이 놓은 6개의 숫자 추출
...
# 3. 각 자리수에서 뽑히는 숫자의 빈도를 가중치로 계산하여 숫자 추출
...
# 4. 각 자리수에서 뽑힌 빈도가 놓은 숫자 중 랜덤 추출



lotto44 <- function(n,x){
...
if(lottofirst(x)==lottosecond(x)==lottothird(x)==lottofourth(x)==lottofifth(x)==lottosixth(x)==lottobonus(x)){



}else{
    a[i,]<- c(lottofirst(x), lottosecond(x), lottothird(x), lottofourth(x), lottofifth(x), lottosixth(x), lottobonus(x))
